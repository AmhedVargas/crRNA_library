...
$num_args = $#ARGV + 1;
if ($num_args != 3) {
  print "\nUsage: Process_arms.pl Genebodies_data.hom.tsv Bed_intersect Column_for_crRNA STDOUT\n\n";
  exit;
}
############
##Define hash tables
...
if($loc > 0){
    ...
    ($leftori[($loc+$dif)+12],$leftori[($loc+$dif+12)+1],$leftori[($loc+$dif+12)+2])=split(//,$E);
...
if($loc > 0){
    ...
    ##Fifth AA
    $e=substr($RightSeq,($loc+$dif)+12,3);
    #print "Fourth codon\n".$d."\n";
    $A=alternate_aa($a);
    #print "First mod codon\n".$A."\n";
    $B=alternate_aa($b);
    ...
    ($rightori[($loc+$dif)+12],$rightori[($loc+$dif+12)+1],$rightori[($loc+$dif+12)+2])=split(//,$E);
